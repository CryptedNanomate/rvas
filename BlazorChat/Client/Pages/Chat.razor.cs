...
namespace BlazorChat.Client.Pages
{
    public partial class Chat
    {
        ...
        //Ovaj dogadjaj proverava da li je korisnik online i da li sadrzaj poruke nije prazan.
        ...
        //proveravamo da li je hub api u funkciji i da li nije povratna vrednost null. ne pocinjemo nista dok nije up.
        protected override async Task OnInitializedAsync()
        {
            ...
            //dodajemo poruku u chat box u realnom vremenu
            hubConnection.On<ChatMessage, string>("ReceiveMessage", async (message, userName) =>
            {
                if ((ContactId == message.ToUserId && CurrentUserId == message.FromUserId) || (ContactId == message.FromUserId && CurrentUserId == message.ToUserId))
                {
                    ...
                    await _jsRuntime.InvokeAsync<string>("ScrollToBottom", "chatContainer");
                    StateHasChanged();
                    //komponenta se rerenderuje ali asinhrono
                }
            });
            await GetUsersAsync();
        ...
        //klikom na chat ova metoda se invokuje
